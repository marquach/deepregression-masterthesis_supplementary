...
# run the first tested setting and calculate
time_first_setting <- 550 # 550 with memory results in 86 hours
sum(time_first_setting*matrix(c(1, 2, 4, 5, 10, 20, 10, 20, 40), byrow = F, nrow = 3)*5)/60/60
...
setting_res <- lapply(X = 1:nrow(settings), function(x){
  ...
  plain_torch_loop <- nn_module(
    ...
    forward = function(input){
      self$intercept_loc(input[,1, drop = F] ) + self$linear(input[,2, drop = F]) + self$nn(input[,3:4, drop = F]) +
  ...
  tictoc::tic()
  benchmark_deepreg_semistruc_mse <- bench::mark(
  ...
  benchmark_deepreg_semistruc_mse$result <- results_mse
  benchmark_deepreg_semistruc_mse
})

names(setting_res) <- setting_names

if(save_res){
save_name <- sprintf("/Users/marquach/Desktop/github/deepregression_master-thesis_supple/speed_comparison/results/speed_comparison_mse_%s.RData",
                     Sys.Date())
save(setting_res, file = save_name)
}

load(file = "/Users/marquach/Desktop/github/deepregression_master-thesis_supple/speed_comparison/results/speed_comparison_mse_2023-08-19.RData")


setting_res_frame <- as.data.frame(Reduce(x = lapply(setting_res, function(x) 
  cbind(min = x[["min"]],
...
benchmark_time <- ggplot(data = setting_res_frame,
  ...
  scale_fill_grey()
benchmark_time
ggsave(filename = paste0(path,"mse-time", ".png"), width = 14, height = 8)


benchmark_time_mse_rel <- ggplot(data = setting_res_frame,
  ...
  geom_text(data = setting_res_frame, aes(x = approach, y = max,
            ...
            vjust = -0.5, size = 3)+
  scale_y_continuous(expand = c(0.05, 0, 0.2, 0))
benchmark_time_mse_rel
ggsave(filename = paste0(path,"mse-time_rel", ".png"), width = 14, height = 8)

library(kableExtra)
library(knitr)
res_appendix <- lapply(1:9, FUN = function(x)
                       t(Reduce(f = cbind,lapply(setting_res[[x]]$time, summary))))
res_appendix <- lapply(1:9, function(x) res_appendix[[x]][c(4,1:3),])
res_appendix <- Reduce(f = rbind, res_appendix)
rownames(res_appendix) <- rep(x = levels(setting_res_frame$approach), 9)
res_appendix

kbl(res_appendix, format = "latex", digits = 2, booktabs = T) %>% 
#  add_header_above(header = c(" " = 1, "mu" = 3, "sigma" = 3)) %>%
  writeLines(paste0(path, "deep-mse_runtime.tex"))

#benchmark_mem_mse_abs <- ggplot(data = setting_res_frame, aes(x = test, y = mem_alloc, fill = test))+
...
#benchmark_mem_mse_abs
#
#benchmark_mem_mse_rel <- ggplot(data = setting_res_frame, aes(x = test, y = mem_alloc, fill = test))+
#  geom_bar(stat = "identity", position = "dodge2")+
#  facet_grid(n ~ epochs,
#             labeller = labeller(
#               epochs = c("100" = "epochs = 100",
